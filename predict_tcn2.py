...
def create_model():
    ...
    x = TCN(nb_filters=G.FILTERS,nb_stacks=G.STACKS,kernel_size=G.KERNEL_SIZE,dilations=(1,2,4,8),padding='causal')(inputs)
...
save_title = '_gru_'+str(G.EPOCHS)+'_'+str(G.LAYERS)+'_'+str(G.FILTERS)+'_dr_'+str(G.DROPOUT)+'_lr_'+str(G.LEARNING_RATE)+'_pred_'+str(G.PRED_SIZE)+'_win_'+str(G.WINDOW_SIZE)
...
if not os.path.exists('tcn/model/'+save_title):
   os.makedirs('tcn/model/'+save_title)

model.compile()
model.save('tcn/model/'+save_title)
