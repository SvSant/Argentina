...
def parse_data_from_file(filename):
    ...
    df = df[['weather_date', 'soil_15D', 'max_TD', 'mean_TD', 'precipD']]
...
def windowed_data(data, window_size, pred_size):
    ...
    for i in range(dim[0]-window_size-pred_size):
        ...
        for j in range(pred_size):
            if j == 0:
                y[i,j] = data[i+window_size+j,0]
            else:
                y[i,j] = y[i,j-1] + data[i+window_size+j,0]
...
def create_model():
    ...
    x = keras.layers.LSTM(G.LSTM_WIDTH, dropout=G.DROPOUT, return_sequences=True)(inputs)
    x = keras.layers.LSTM(G.LSTM_WIDTH, dropout=G.DROPOUT, return_sequences=True)(x)
    x = keras.layers.LSTM(G.LSTM_WIDTH, dropout=G.DROPOUT, return_sequences=True)(x)
    # x = keras.layers.LSTM(G.LSTM_WIDTH, dropout=G.DROPOUT, return_sequences=True)(x)
    x = keras.layers.LSTM(G.LSTM_WIDTH, dropout=G.DROPOUT)(x)
...
save_title = '_LSTM_'+str(G.EPOCHS)+'_'+str(G.LAYERS)+'_'+str(G.LSTM_WIDTH)+'_dr_'+str(G.DROPOUT)+'_lr_'+str(G.LEARNING_RATE)+'_pred_'+str(G.PRED_SIZE)+'_win_'+str(G.WINDOW_SIZE)
...
for epoch in range(1,G.EPOCHS+1):
    ...
    np.savetxt('lstm/loss_history/train_loss_'+save_title+'.csv', train_loss_plot)
    np.savetxt('lstm/loss_history/test_loss_'+save_title+'.csv', test_loss_plot)
...
# make plot title
plot_title = 'Epochs = '+str(G.EPOCHS)+', pred_days = '+str(G.PRED_SIZE)+'\n Layers = '+str(G.LAYERS)+', Width = '+str(G.LSTM_WIDTH)+', Learning_rate = '+str(G.LEARNING_RATE)+', Dropout = '+str(G.DROPOUT)+'\n MSE = '+str(mse_all)+', MAE = '+str(mae_all)

if not os.path.exists('lstm/predictions/'+save_title):
   os.makedirs('lstm/predictions/'+save_title)

if not os.path.exists('lstm/model/'+save_title):
   os.makedirs('lstm/model/'+save_title)

model.compile()
model.save('lstm/model/'+save_title)
...
plt.savefig('lstm/loss_history/loss'+save_title+'.png')
plt.close()

for i in range(G.PRED_SIZE):
    plt.figure(figsize=(20,10))
    plt.plot(X_plot_test, y_test[:,i], label='Soil moisture - true', color='black', lw = 0.5)
    plt.plot(X_plot_test, y_pred_test[:,i], label='Soil moisture - predicted', color='blue', lw = 0.5)
    plt.legend()
    plt.xlabel('Time')
    plt.ylabel('Soil moisture (normalized)')
    plt.title('Soil moisture - day '+str(i+1)+'\n'+plot_title)
    plt.savefig('lstm/predictions/'+save_title+'/test_day_'+str(i+1)+'.png')
    plt.close()
...
plt.savefig('lstm/predictions/'+save_title+'/test_error.png')
plt.close()
